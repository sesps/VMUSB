...
puts "THIS IS AN OFFLINE VERSION... READING THE FILE IN SAME DIRECTORY AS SCRIPT"
#if {[catch {configRead [file join ~ config  daqconfig.tcl]} msg]} {
if {[catch {configRead $here/daqconfig.tcl} msg]} {
...
#--ddc sep12 set the 'histogram channels' for the 'marker'
set channelCount($typeMARKER)  16;	# bit mask for a marker
...
proc buildHINPMap {param module} {
	...
	#--ddc changed channel count from 16 to 32 to match 'legacy'
...
proc buildChannelMaps param {
	...
	} elseif {$::readoutDeviceType($module) eq $::typeMARKER} { 
#--ddc sep12 put in marker mapping.
	    puts "MARKER $module"
	    set param [buildMARKERMap $param $module]
	} else {
...
#--ddc sep12 map the MARKER, so it can be unpacked.
#---------------------------------------------------------------------------
# Build the channel maps and spectrum
# We're going to build a parameter map with only one parameter for the marker
# and a bit map spectrum for the device.
# Parameters:
#    param   - Number of the first usable parameter.
#    name    - module name
...
#    Number of next usable parameter.
#
proc buildMARKERMap {param module} {
