		  ...
		  CParamMapCommand::AdcMapping* pMap)
{
  ...
  for (int i =0; i < (wordCount-7)/5; i++) {
  // get channel tag
    channelId      = getLong(event, offset);
    offset += 2;
    // now pick up analog data
    uint32_t a     = getLong(event, offset) & 0xfff;
    offset += 2;
    uint32_t b     = getLong(event, offset) & 0xfff;
    offset += 2;
    uint32_t c     = getLong(event, offset) & 0xfff;
    offset += 2;
    uint32_t time  = getLong(event, offset) & 0xfff;
    offset += 2;

    uint32_t channel   = channelId & 0x0f;
    uint32_t chip      = (channelId >> 5) & 0xff;
    printf("chip %d chan %d A %d B %d C %d T %d\n",chip, channel,a,b,c,time);
